...
for i=1:length(mfiles)
%     title = strsplit(names{i},'/');
%     title = title{end};
    item = containers.Map(...
        {'id', 'title', 'synopsis', 'description'},...
        {fullurl(mdirs{i}, names{i}), names{i}, synopsis{i}, h1line{i} } ...
        );
%     item('links') = containers.Map(...
%          {'docs'},{fullurl(mdirs{i},names{i})});
    json_data(names{i}) = item;
end
...
for i=1:prod(size(ind))
	if ind(i)
		tpl = set(tpl,'var','L_IDNAME',...
			fullurl(mdirs{ind(i)},[names{ind(i)}]));
%cmp		fullurl(mdirs{ind(i)},[names{ind(i)} options.extension]));
...
%cmp	tpl = set(tpl,'var','INDEX',[options.indexFile options.extension]);
	tpl = set(tpl,'var','MASTERPATH','./');
	tpl = set(tpl,'var','DATE',[datestr(now,8) ' ' datestr(now,1) ' ' ...
								datestr(now,13)]);
...
for i=1:length(mdir)
	...
	for j=1:length(mdirs)
		if strcmp(mdirs{j},mdir{i})
			tpl = set(tpl,'var','L_NAME', [names{j}]);
%cmp		tpl = set(tpl,'var','L_NAME', [names{j} options.extension]);
	...
	for j=1:length(d)
		if ismember(fullfile(mdir{i},d{j}),mdir)
			tpl = set(tpl,'var','SUBDIRECTORY',...
				...
				fullurl(d{j}),d{j}));
%cmp			fullurl(d{j},[options.indexFile options.extension]),d{j}));
		else
